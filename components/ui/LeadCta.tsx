...
export function LeadCta({ label = "Quero minha simulação gratuita", size = "lg", className, location = "page" }: LeadCtaProps) {
  ...
  const padding = size === "lg" ? "pl-8 pr-2.5 py-2.5 text-lg" : "pl-6 pr-2 py-2 text-base";
  ...
  return (
    <button
      type="button"
      onClick={handleClick}
      className={`group inline-flex items-center gap-4 whitespace-nowrap rounded-full gold-gradient ${padding} font-heading font-bold text-pe-slate-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.35),0_6px_36px_rgb(212_122_0/0.4)] hover:brightness-105 hover:scale-105 transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-pe-solar-400 focus-visible:ring-offset-2 ${className ?? ""}`}
    >
      {label}
